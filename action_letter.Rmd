...
In fact, there is so much work on this topic that it would be a tall order to review the literature on an individual study basis.
Given the large prior literature,
we rely primarily on prior review articles.
We currently cite 17 review articles in the introduction,
including a recent (2021) comprehensive systematic review of 6305 studies on factors associated with adolescent emotion regulation.
...
and seems overly ambitious, given the page limits and attention that must be devoted to explaining the novel methods,
...
Of course we are willing to include specific omitted literature, but please note that Reviewer 3 does not mention which references they feel are missing.
...
As mentioned above, the vast majority of cited studies are reviews (narrative reviews, meta-analyses, and book chapters) or theoretical publications.
...
To address this comment, we have incorporated the following in our analysis of the results in light of the bioecological model:
...
The specific parenting practices associated with instilling a sense of balanced relatedness are maternal (54) and paternal (55) support, which rank very low.
...
This is consistent with the finding that some parent-related constructs indeed score highly.
...
We thank the Editor for this excellent suggestion; we have addressed this comment by adding the following paragraph:
...
```{r figviolin, results='asis', out.width="20%", fig.cap="Variable importances clustered by theme (A) and level of the bioecological model (B).", eval = FALSE}
...
`r get_revision(manuscript = man, "examplenonlinear")`
...
We apologize for the inconvenience. The manuscript only has 42 pages; pages 43-65 are copies of the online supplemental materials (which are supposed to be accessed via the broken links, which have now been fixed). For the sake of blind review, we also submitted copies of these tables through the submission portal, but it evidently cannot correctly present these materials. The tables will not be part of the paper in this form.
...
We also believe that the Figure above helps clarify these results.
...
whereas SEM-forests *can* answer this question, because cases are clustered based on predictor variables (of course with the caveat that causal interpretation is contingent on study design or theoretical justification).
...
The Reviewer is citing cutting-edge developments in the area of classic random forests; it will take a while before these developments spill over into the SEM-forests literature, if ever.
...
We apologize for the inconvenience. These Supplemental Tables will not be part of the printed manuscript; we were required to attach hard copies of the online supplemental tables in the submission portal, and these were evidently not formatted correctly. They are just spreadsheets that can be opened in a web-browser or excel.
...
We appreciate these suggestions and the caution about abstract length; we have cut the abstract by half and attempted to incorporate the Reviewer's suggestions. The Abstract now reads:
...
`r get_revision(manuscript = man, "personcentered1")`
...
At present, we cite a total of 17 review articles in the introduction,
including a recent (2021) comprehensive systematic review of 6305 studies on factors associated with adolescent emotion regulation.

Given the large prior literature,
we found it necessary to rely primarily on review articles to do justice to prior work.
We cannot strive to recreate a comprehensive review here - 
and this also seems redundant, given that many of the cited review articles are very recent (i.e., from 2019-2021),
and seems overly ambitious, given the page limits and attention that must be devoted to explaining the novel methods,
and spurious, given that this is an exploratory study and thus has no hypotheses that must be supported with prior literature.
...
Of course we are willing to include **specific** omitted literature, but it is not clear from the comment which specific references should be included.
...
Comprehensive descriptive statistics about all available demographic characteristics are reported in Tables 1 and 2.
To address this comment, we now stress this in the Participants section:
...
In line with the Reviewer's expectation, we did in fact use the median and 16th and 84th percentile.
The confusion arises from our attempt to explain to the readership what these percentiles mean, by drawing a comparison to +/- 1SD.
...
`r get_revision(manuscript = man, "highrankingparent", quote = F)`
