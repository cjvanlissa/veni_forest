...
`r get_revision(man, "predictions")`
...
`r get_revision(man, "examplenonlinear")`
...
`r get_revision(man, "limitations")`
...
`r get_revision(man, "includepersonality")`
...
We understand why the Reviewer expects the analysis to include model tuning and cross-validation; these are standard procedures in machine learning.
...
It is important to note, however, that unlike other techniques, random forests typically achieve competitive performance even with default settings [@probstHyperparametersTuningStrategies2019].
Thus, even when a model isn't tuned - or cannot be tuned, as is the case here - it should perform well.
...
We thank the Reviewer for this excellent suggestion, and now provide this figure
...
We appreciate this point, and the writing here should certainly be more nuanced (that was an oversight).
We have rephrased this as follows:

`r get_revision(man, "riskassessment")`
