...
abstract: |
  ...
  | • Parenting may be less relevant for emotion regulation development than previously thought, and negative parenting may be more impactful than positive parenting.
...
<span id = "abundantlit">There is already an abundance of empirical research on emotion regulation and its predictors; a recent systematic review identified 6305 relevant publications [@vanlissaMappingPhenomenaRelevant2021].
Still, that review identified specific limitations of the existing literature: many studies are narrow in scope, considering only a few predictors and thus missing the relative importance compared to other predictors.</span>
This problem is exacerbated because the literature is fragmented across sub-disciplines, each of which examines mostly predictors within its scope.
And finally, the literature lacks a unifying theoretical framework that is both specific to adolescence and comprehensive in scope [see @bariolaChildAdolescentEmotion2011].
...
Specifically,
<span id = "personcentered1">the SEM-forests algorithm [@brandmaierTheoryguidedExplorationStructural2016] was used as a person-centered analysis technique,
...
The resulting insights indicate which factors are most predictive of adolescents' diverging trajectories of emotion regulation development, contributing to our understanding of why some youth flourish while others struggle [@croneUnderstandingAdolescencePeriod2012].</span>
...
Although some studies have examined, on average, how strongly particular risk factors are associated with developmental trajectories 
...
[Note that the RMSEA has been found to be more sensitive to misspecifications of growth curve models [@leiteDetectingGrowthShape2011]]{#cfi}.
...
[For continuous predictors, three values of $s$ were used - namely the percentiles, computed on the observed data, corresponding to the median, $+/- 1 SD$ of a standard normal distribution (i.e., the $16^{th}$ and $84^{th}$ percentile).
For convenience, these percentiles are labeled as $+/- 1 SD$ in Figure \@ref(fig:figpdp1)-\@ref(fig:figpdp4).]{#percentiles}
...
Finally, an emerging theme is that internalizing- and externalizing symptoms
often associated with emotion regulation ranked moderately in terms of predictive importance.
...
Themes that ranked notably low were family demographics (SES and religion) and bullying, victimization and delinquency, and substance use.
...
and yet research on adolescents does not routinely assess the role of personality alongside factors of particular interest in our discipline, such as parenting, peer relationships, and problem behavior [@vanlissaMappingPhenomenaRelevant2021].
...
In cross-cultural comparisons, the Netherlands has been found (on average) to ascribe to similar cultural values as large Anglophonic countries (e.g., the United States, Canada, and Australia) [@terlutterGLOBEStudyApplicability2006].
